<div class="wrap">
    ...
    <div class="tab-content">
        ...
        <?php 
				$options = get_option('mhamdy_plugin_cpt') ?: []; // if not $option then $option will equall []
				...
				foreach ($options as $option) {
                    ...
                    settings_fields( 'mhamdy_plugin_cpt_settings' );
                    echo '<input type="hidden" name="remove" value="'.$option['post_type'].'">';
                    submit_button('Delete', 'delete small', 'submit', false, [
                        'onclick' => 'return confirm("Are you sure you want to delete this custom post type? The data associated with it wont be deleted. ");'
                    ]);
        ...
        <div id="tab-2" class="tab-pane <?php echo (isset($_POST['edit_post'])) ? 'active' : '' ?>">
            ...
            <form method="post" action="options.php">
                <?php 
                    settings_fields( 'mhamdy_plugin_cpt_settings' );
                    do_settings_sections( 'mhamdy_cpt' );
                    submit_button();
                ?>
            </form>
        </div>
